...
def queriesView(request, query_num):
  ...
  if query_num == 1 :
    ...
    query_string = 'SELECT "crisis_ID", name, common_summary FROM wcdb_crisis WHERE "crisis_ID" in (SELECT model_id FROM (SELECT model_id, count(*) FROM wcdb_li WHERE kind = $$Citations$$ GROUP BY model_id HAVING count(*) >= 3) AS counts);'
    query_result_set = Crisis.objects.raw(query_string)

  elif query_num == 3 :
    query_string =  'SELECT id, model_id, floating_text, href FROM wcdb_li WHERE model_id in (SELECT "org_ID" FROM (SELECT "org_ID", count(*) FROM wcdb_relations WHERE "org_ID" <> $$$$ AND "crisis_ID" <> $$$$ GROUP BY "org_ID" HAVING count(*) >= 2) AS counter) AND kind = $$ContactInfo$$;'
    query_string1 = 'SELECT id, model_id FROM wcdb_li WHERE model_id in (SELECT "org_ID" FROM (SELECT "org_ID", count(*) FROM wcdb_relations WHERE "org_ID" <> $$$$ AND "crisis_ID" <> $$$$ GROUP BY "org_ID" HAVING count(*) >= 2) AS counter) AND kind = $$ContactInfo$$;'
    query_string2 = 'SELECT DISTINCT model_id FROM wcdb_li WHERE model_id in (SELECT "org_ID" FROM (SELECT "org_ID", count(*) FROM wcdb_relations WHERE org_ID <> $$$$ AND crisis_ID <> $$$$ GROUP BY "org_ID" HAVING count(*) >= 2) AS counter) AND kind = $$ContactInfo$$;'
    ...
    query_string =  'SELECT "org_ID", name FROM wcdb_org WHERE "org_ID" IN (SELECT model_id FROM (SELECT model_id, count(*) FROM wcdb_li WHERE kind = $$Videos$$ GROUP BY model_id HAVING count(*) >= 2) AS counter);'
    query_result_set = Org.objects.raw(query_string)

  elif query_num == 8 :  
    query_string =  'SELECT "person_ID", name FROM wcdb_person WHERE "person_ID" IN (SELECT "person_ID" FROM (SELECT "person_ID", count(*) FROM wcdb_relations WHERE "person_ID" <> $$$$ AND "crisis_ID" <> $$$$ GROUP BY "person_ID" HAVING count(*) >= 2) AS counter);'
    query_result_set = Person.objects.raw(query_string)
